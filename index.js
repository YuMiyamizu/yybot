...
const start = async (kill = new Client()) => {
	const getversion = await axios.get('https://raw.githubusercontent.com/YuMiyamizu/ybot/main/package.json')
	if (irisvs.version !== getversion.data.version) { console.log(color('\n[UPDATE]', 'crimson'), color(`Uma nova versão do YBOT foi lançada [${getversion.data.version}], atualize para obter melhorias e correções! → ${ybotvs.homepage}`, 'gold')) }
	console.log(color('\n[SUPORTE]', 'magenta'), color(`wa.me//+5511960930541 | ${ybotvs.bugs.url}\n`, 'lime'), color(`\n[YBOT ${ybotvs.version}]`, 'magenta'), color('Estamos prontos para começar mestre!\n', 'lime'))
