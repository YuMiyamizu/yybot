...
*
* 						Obrigado a você que escolheu ao YBOT.
*
* ---------------------------------------------------------------------------------------------------*/
...
module.exports = kconfig = async (kill, message) => {
    ...
    try {
		...
		const YBotMsgs = fs.readFileSync('./lib/config/Utilidades/reply.txt').toString().split('\n')
		const chatBotR = yBotMsgs[Math.floor(Math.random() * yBotMsgs.length)].replace('%name$', `${name}`).replace('%battery%', `${lvpc}`)
		...
		case 'about':
			await kill.sendFile(from, './lib/media/img/ybot.png', 'ybot.png', mess.about(), id)
			break
        ...
        case 'nobg':
			if (isMedia && type === 'image' || isQuotedImage) {
				...
				await kill.sendImageAsSticker(from, `data:${nobgmd.mimetype};base64,${result.base64img}`, { author: 'Yuri', pack: 'Yuri', keepScale: true })
        ...
        case 'ybot':
			try {
				if (args[0] == '-g') {
					...
					const ybot = await axios.get(`http://simsumi.herokuapp.com/api?text=${encodeURIComponent(body.slice(6))}&lang=${config.lang}`)
					if (ybot.data.success == 'Limit 50 queries per hour.' || ybot.data.success == '' || ybot.data.success == null) {
						await kill.reply(from, chatBotR, id)
					} else {
						if (ybot.data.success == 'INDISP') return kill.reply(from, '....', id)
						await kill.reply(from, ybot.data.success, id)
						fs.appendFile('./lib/config/Utilidades/reply.txt', `\n${ybot.data.success}`)
					}
				}
			} catch (error) { 
				await kill.reply(from, chatBotR, id)
				console.log(color('[YBOT]', 'crimson'), color(`→ Obtive erros no comando ${prefix}${command} → ${error.message} - Você pode ignorar.`, 'gold'))
			}
			break
        ...
        case 'speak':
			...
			try {
				if (args[0] == '-g') {
					...
					const spybot = await axios.get(`http://simsumi.herokuapp.com/api?text=${encodeURIComponent(body.slice(7))}&lang=${config.lang}`)
					const a = spybot.data.success
